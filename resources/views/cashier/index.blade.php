  ...
  <form action="{{ route('cashier') }}" method="GET" class="flex items-center justify-end gap-4 px-8 py-4">
    ...
    <div class="relative -space-x-2" x-data="{ open: false }">
      <button @click="open = !open" type="button"
        class="{{ request('sort') ? 'bg-button-main text-gray-600 hover:bg-button-hover' : 'border-gray-300 text-gray-400 hover:bg-gray-100' }} rounded-lg border-2 p-1.5 transition-colors">
    ...
    <div class="relative w-80">
      ...
      <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Produk"
        class="focus:border-button-main w-full rounded-lg border-2 border-gray-300 py-1 pl-10 pr-4 transition-colors focus:outline-none">
    </div>
  </form>
      ...
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 2xl:grid-cols-3">
        @foreach ($products as $product)
          <button
            ...
            class="border-3 w-full rounded-2xl p-4 text-left transition-all hover:shadow-lg hover:scale-[1.01]">

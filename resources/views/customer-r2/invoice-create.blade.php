<x-app-layout>
    <div x-data="manualInvoiceForm()" class="py-4 lg:py-6 font-mont">
        <div class="mx-auto w-full px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="POST" action="{{ route('customer-r2.invoice.store', $customer->id) }}" @submit.prevent="submitForm($event)">
                @csrf
                                        ...
                                        <template x-for="(item, idx) in cart" :key="item.id">
                                            <tr class="border-t border-gray-100 hover:bg-gray-50/40 transition-colors">
                                                ...
                                                <td class="px-3 py-3 text-right">
                                                    <input type="number" min="0" step="1" x-model.number="item.price"
                                                        class="w-28 rounded-md border border-gray-200 px-2 py-1 text-right text-sm tabular-nums focus:outline-none focus:border-button-main focus:bg-white">
                                                </td>
            ...
            function manualInvoiceForm() {
                return {
                    ...
                    submitForm(e) {
                        ...
                        append(f, '_token', form.querySelector('input[name="_token"]').value);
